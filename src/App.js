...
function buildOrderReceiptHTML(sale, biz, logo) {
  ...
  return `${headerHTML(
    ...
    sale.deliveryFee > 0
      ...
      : ""
  }</tbody></table></div><div class="totals-block"><div class="totals-inner"><div class="total-row"><span class="label">Order Total</span><span class="value">${fmt(total)}</span></div><div class="total-row highlight"><span class="label">${amountLabel}</span><span class="value">${fmt(paid)}</span></div>${isPartial ? `<div class="total-row"><span class="label" style="color:#dc2626">Balance Due</span><span class="value" style="color:#dc2626;font-weight:800">${fmt(balance)}</span></div>` : `<div class="total-row grand"><span class="label">TOTAL PAID</span><span class="value">${fmt(paid)}</span></div>`}</div></div>${sale.notes ? `<div class="notes-box"><strong>Order Notes:</strong> ${sale.notes}</div>` : ""}<div class="notes-box"><strong>${isPartial ? "Partial payment received. Please settle the remaining balance." : "Thank you!"}</strong>${isPartial ? "" : " We appreciate your order. Come back soon!"}</div>${biz.paymentTerms ? `<div class="terms-box"><p><strong>Payment accepted via:</strong> ${biz.paymentTerms.replace(/\n/g, "<br>")}</p></div>` : ""}${footerHTML(biz)}`;
}
...
function buildOrderInvoiceHTML(sale, biz, logo) {
  ...
  return `${headerHTML(biz, logo, "INVOICE", invNum, sale.date, sale.date)}${partiesHTML(
    ...
    "Bill To",
    `<div class="party-name">${clientName}</div><div class="party-detail">${clientDetail}</div>`
  )}${statusBanner}<div class="items-section"><div class="section-heading">Order Line Items</div><table><thead><tr><th>Description</th><th class="tc">Qty</th><th class="tr">Unit Price</th><th class="tr">Total (XAF)</th></tr></thead><tbody>${rows}</tbody></table></div><div class="totals-block"><div class="totals-inner"><div class="total-row"><span class="label">Subtotal</span><span class="value">${fmt(sale.plates * sale.pricePerPlate)}</span></div>${sale.deliveryFee > 0 ? `<div class="total-row"><span class="label">Delivery Fee</span><span class="value">${fmt(sale.deliveryFee)}</span></div>` : ""}<div class="total-row grand"><span class="label">ORDER TOTAL</span><span class="value">${fmt(total)}</span></div><div class="total-row highlight"><span class="label">Amount Paid</span><span class="value">${fmt(paid)}</span></div><div class="total-row"><span class="label" style="${balance>0?'color:#dc2626':'color:#16a34a'}">Balance Due</span><span class="value" style="${balance>0?'color:#dc2626;font-weight:800':'color:#16a34a;font-weight:800'}">${fmt(balance)}</span></div></div></div><div class="payment-box"><h4>Payment Information</h4><div class="payment-row"><span>Method:</span><strong>${sale.method}</strong></div><div class="payment-row"><span>Date:</span><strong>${sale.date}</strong></div><div class="payment-row"><span>Order type:</span><strong>${sale.type}</strong></div>${sale.notes ? `<div class="payment-row"><span>Notes:</span><strong>${sale.notes}</strong></div>` : ""}</div><div class="terms-box">${balance > 0 ? `<p>A balance of <strong>${fmt(balance)}</strong> XAF remains outstanding. Please arrange payment at your earliest convenience.</p>` : ""}${biz.paymentTerms ? `<p><strong>Payment accepted via:</strong> ${biz.paymentTerms.replace(/\n/g, "<br>")}</p>` : ""}</div>${footerHTML(biz)}`;
}
              ...
              <div style={{ ...S.row, justifyContent: "space-between", marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={{ ...S.btn("ghost"), fontSize: 11 }} onClick={() => {
                    ...
                    const { detail: bizDetail } = getBizHTML(biz, logo);
                    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Statement — ${selCustomer.name}</title>
<style>body{font-family:sans-serif;color:#111;padding:40px;max-width:800px;margin:0 auto}h1{font-size:22px;margin:0}h2{font-size:14px;font-weight:700;margin:24px 0 8px;border-bottom:2px solid #111;padding-bottom:4px}table{width:100%;border-collapse:collapse;margin-bottom:16px}th{background:#111;color:#fff;padding:7px 10px;font-size:11px;text-align:left}td{padding:7px 10px;border-bottom:1px solid #eee;font-size:12px}.ar{color:#dc2626;font-weight:700}.paid{color:#16a34a;font-weight:600}.total-row{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eee;font-size:13px}.grand{font-weight:800;font-size:15px;border-top:2px solid #111;margin-top:4px;padding-top:8px}.badge{display:inline-block;background:#ddd;border-radius:12px;padding:2px 10px;font-size:11px;color:#333;font-weight:600}</style></head><body>
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;border-bottom:2px solid #111;padding-bottom:20px">
  <div><div style="font-size:14px;font-weight:700;color:#111">${biz.name || "Cookie's Bites"}</div><div style="font-size:11px;color:#444;margin-top:3px;line-height:1.5">${bizDetail}</div></div>
  <div style="text-align:center"><img src="${(logo?.src || LOGO_SRC)}" alt="logo" style="width:225px;height:225px;object-fit:contain;display:block;margin:0 auto"/></div>
  <div style="text-align:right"><h1 style="color:#111;font-size:18px;margin:0 0 6px">Customer Statement</h1><p style="margin:2px 0;color:#444;font-size:12px">${new Date().toLocaleDateString("fr-CM",{day:"2-digit",month:"long",year:"numeric"})}</p><div style="margin-top:8px;border-top:1px solid #eee;padding-top:8px"><strong style="font-size:15px;color:#111">${selCustomer.name}</strong><br/><span class="badge">${selCustomer.classification}</span><br/>${selCustomer.phone ? `<span style="font-size:11px;color:#666">${selCustomer.phone}</span><br/>` : ""}${selCustomer.email ? `<span style="font-size:11px;color:#666">${selCustomer.email}</span>` : ""}</div></div>
</div>
<div style="background:#f0f0f0;border-radius:8px;padding:14px 20px;margin-bottom:24px;display:flex;gap:40px">
  <div><div style="font-size:11px;color:#444;font-weight:700;text-transform:uppercase;letter-spacing:1px">Total Spend</div><div style="font-size:22px;font-weight:800;color:#16a34a">${fmt(totalSpend)}</div></div>
  ${totalAR > 0 ? `<div><div style="font-size:11px;color:#444;font-weight:700;text-transform:uppercase;letter-spacing:1px">Outstanding AR</div><div style="font-size:22px;font-weight:800;color:#dc2626">${fmt(totalAR)}</div></div>` : `<div><div style="font-size:11px;color:#444;font-weight:700;text-transform:uppercase;letter-spacing:1px">Outstanding AR</div><div style="font-size:22px;font-weight:800;color:#16a34a">✅ Fully Paid</div></div>`}
  <div><div style="font-size:11px;color:#444;font-weight:700;text-transform:uppercase;letter-spacing:1px">Orders</div><div style="font-size:22px;font-weight:800;color:#111">${cSales.length}</div></div>
</div>
